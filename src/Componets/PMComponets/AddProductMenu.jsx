       ...
       {showUpload && <Uploading per={showUpload} setUpload={setShowUpload}/>}
        <div className="relative shadow-md shadow-black bg-slate-400 rounded-3xl h-[32rem] w-[90%] m-auto">
            ...
            <div className=' m-auto w-[90%] h-[70%] relative'>
                <input  name={'img'} id='upload' type='file' onChange={({target})=>{uploadfile(target.name, setProductData, setShowUpload )}} accept="image/png, image/jpeg" hidden/>
               ...
               <button className='h-14 w-[80%] mx-8 my-4 bg-blue-600 text-white font-bold text-3xl rounded-full' onClick={()=>{addToShop(productData)}}>Add to Shop</button>
            </div>
